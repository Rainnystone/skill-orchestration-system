...
def _selected_manifests(
    ...
    for pack_id in pack_ids:
        safe_pack_id = safe_component(pack_id, "pack_id")
...
def _validate_workspace_activation_plan(
    ...
    if required_path(learned_operation.target) != learned_target:
...
def _validate_workspace_skill_operation(
    ...
    *,
    ...
    safe_skill_name = safe_pointer_skill(metadata_skill_name)
...
def _validate_pointer_operations(
    ...
    *,
    ...
    for operation, manifest in zip(operations, manifests, strict=True):
        ...
        if safe_component(pack_id, "pack_id") != manifest.id:
        ...
        if safe_pointer_skill(pointer_skill) != manifest.pointer_skill:
...
def _validate_workspace_skill_target(
    ...
    ensure_under(target, workspace_skill_root, "workspace skill target path")
    if target.name != "SKILL.md":
        raise ValueError(f"workspace skill target must be SKILL.md: {target}")
    skill_name = target.parent.name
    if safe_pointer_skill(skill_name) != expected_skill_name:
...
def _snapshot_targets(
    ...
    rollback_paths = unique_paths(
        (
            validated.workspace_skill_root.parent,
            validated.learned_reference_target.parent,
        )
    )
    return snapshot_paths(rollback_paths, prefix="sos-workspace-activation-")
