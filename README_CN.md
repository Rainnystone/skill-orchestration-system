...
**A**: SOS 设计了一道主机适配器接缝 (host adapter seam)，把宿主专属行为集中在一处，而不是在代码里到处散落 `if host == "codex"` 的判断：
- **Codex**：适配器会修改 Codex 专用的 skill 配置文件来启用/禁用技能。禁用步骤在 pointer skills 渲染*之后*才执行，这样即便 apply 中途被打断，也不会出现“原技能已禁用、SOS 指针却还没渲染出来”的尴尬局面。
- **Claude Code**：由于 Claude Code 没有显式配置文件，适配器会把禁用的技能文件夹移动到宿主目录下的 `.sos-archive/` 目录中，让 Claude 无法发现它们，从而达成禁用效果。
...
|-- src/sos/                # CLI 和库实现
|   |-- cli.py              # 薄命令行入口（仅 handler）
|   |-- scanner.py          # SKILL.md 发现
|   |-- propose.py          # pack 建议规则
|   |-- pack_inspect.py     # 只读 pack list/show helpers 和指纹
|   |-- changes.py          # 只读 runtime 和 skill drift 报告
|   |-- planner.py          # 可审查写入计划
|   |-- plan_ops.py         # 共享 plan 操作查询助手
|   |-- apply.py            # 计划执行和可回滚写入
|   |-- host_adapter.py     # 主机适配器接缝（Codex/Claude 行为）
|   |-- fs_transaction.py   # 共享快照/回滚文件系统原语
|   |-- path_safety.py      # 共享路径组件与根包含校验器
|   |-- pointer.py          # active skill pointer 渲染
|   |-- manifest.py         # pack manifest 与 registry I/O
|   |-- backups.py          # 备份创建与公共门面
|   |-- backup_records.py   # 备份记录管理（元数据、列表）
|   |-- backup_restore.py   # 备份恢复与清理执行
|   |-- sync.py             # pack sync 规划与激活
|   |-- redaction.py        # 面向用户输出的本地路径脱敏
...
SOS 通过一道主机适配器接缝支持两种宿主：

- **Codex**：写入路径在创建备份后更新 Codex skill 配置，且只有传入 `--apply` 时才会写入。配置禁用步骤在 pointer skills 渲染之后执行，这样两步之间即便被打断，也不会出现原技能已禁用却还没渲染出 SOS 指针的情况。
